...
let getQualifierRange (element: ITreeNode) =
    match element with
    | :? IReferenceExpr as refExpr -> getTreeNodesDocumentRange refExpr refExpr.Delimiter
    | :? IReferenceName as referenceName -> getTreeNodesDocumentRange referenceName referenceName.Delimiter
